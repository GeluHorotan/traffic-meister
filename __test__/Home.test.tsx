...
describe("Homepage", () => {
  ...
  test("renders one VehicleCard for each item in mockData", async () => {
    render(<Home />);

    await waitFor(async () => {
      const vehicleCards = await screen.getAllByTestId("vehicle-card");
      expect(vehicleCards).toHaveLength(mockData.length);
    });
  });
});
